...
`protected _submitButton: HTMLButtonElement` - элемент разметки шаблона формы в модальном окне в котором хранится кнопка типа submit.
...
`disableFormButton(value: boolean)` - устанавливает доступность кнопки формы `_submitButton` в зависимости от переданного параметра.
...
`contacts:submit` - событие для отправки данных на сервер и открытия модального окна с успешным оформлением заказа, генерируется при нажатии на кнопку "оплатить" на форме контактов.
...
`order:submit` - событие для перехода к шагу с заполнением формы контактов в случае успешной проверки данных.
...
#### Слушаетель события order:submit
...
#### Слушаетель события contacts:submit
